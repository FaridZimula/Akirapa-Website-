                        ...
                        <form onSubmit={handleSubmit} className="space-y-10">
                            ...
                            <div className="space-y-6">
                                    ...
                                    {(provider === "card" ? usdPresets : presets).map((val) => (
                                        <button
                                            ...
                                            className={`h-14 rounded-xl border-2 transition-all font-bold text-lg ${selectedPreset === val
                                                ...
                                                : "border-gray-100 text-gray-600 hover:border-blue-100 hover:bg-blue-50/30"
                                                }`}
                                        >
                                            {getSymbol()} {val >= 1000 ? `${val / 1000}k` : val}
                                            {(val === 50000 || val === 50) && (
                                                <span className={`block text-[10px] uppercase tracking-wider -mt-1 font-bold ${selectedPreset === val ? "text-blue-100" : "text-blue-600"}`}>Suggested</span>
                                            )}
                                        </button>
                                    ))}
                                    <button
                                ...
                                <div className="flex justify-center group">
                                    <div className="relative w-full max-w-sm">
                                        <div className="absolute left-6 top-1/2 -translate-y-1/2 text-xl font-bold text-gray-400 group-focus-within:text-blue-600 transition-colors">{getSymbol()}</div>
                                        <Input
                                            id="custom-amount"
                                            ref={inputRef}
                                            type="number"
                                            placeholder="0.00"
                                            className="h-20 pl-20 pr-8 text-center text-3xl font-bold rounded-2xl border-2 border-gray-100 focus:border-blue-600 bg-gray-50/30 focus:bg-white transition-all outline-none ring-0 focus-visible:ring-0 text-blue-600"
                                            value={amount}
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    {/* Card Option */}
                                    <div className={`rounded-2xl border-2 transition-all overflow-hidden ${provider === "card" ? "border-primary ring-4 ring-primary/5 shadow-md" : "border-gray-100 hover:border-gray-200"}`}>
                                        <button
                                            type="button"
                                            onClick={() => setProvider("card")}
                                            className="w-full flex items-center justify-center gap-4 p-6"
                                        >
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${provider === "card" ? "border-primary" : "border-gray-300"}`}>
                                                {provider === "card" && <div className="w-3 h-3 bg-primary rounded-full" />}
                                            </div>
                                            <div className="flex flex-col items-center gap-1">
                                                <span className="font-bold text-gray-900 text-lg">Credit or debit card</span>
                                            </div>
                                        </button>

                                        {provider === "card" && (
                                            <div className="px-6 pb-8 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300 border-t border-gray-50 pt-6">
                                                <div className="space-y-2">
                                                    <Label className="text-xs font-bold uppercase tracking-wider text-gray-400">Card Information</Label>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        >
                                            ...
                                            <span className={`font-bold text-sm ${provider === "mtn" ? "text-yellow-700" : "text-gray-500 group-hover:text-yellow-600"}`}>MTN MoMo</span>
                                            {provider === "mtn" && <div className="absolute top-3 right-3 w-5 h-5 bg-yellow-400 rounded-full flex items-center justify-center text-white ring-2 ring-white"><CheckCircle2 className="w-4 h-4" /></div>}
                                        </button>
                                        ...
                                        >
                                            ...
                                            <span className={`font-bold text-sm ${provider === "airtel" ? "text-red-700" : "text-gray-500 group-hover:text-red-600"}`}>Airtel Money</span>
                                            {provider === "airtel" && <div className="absolute top-3 right-3 w-5 h-5 bg-red-600 rounded-full flex items-center justify-center text-white ring-2 ring-white"><CheckCircle2 className="w-4 h-4" /></div>}
                                        </button>
                                    </div>

                                    {provider && provider !== "card" && (
                                        <div className="p-6 rounded-2xl bg-gray-50 border border-gray-100 animate-in slide-in-from-top-2 duration-300">
                                            <div className="space-y-4 flex flex-col items-center">
                                                <Label htmlFor="phone" className="text-sm font-bold text-gray-700 text-center">Mobile Money Number</Label>
                                                ...
                                                <p className="text-xs text-gray-400 italic text-center">You'll receive a secure PIN prompt on your phone.</p>

                                                <div className="w-full pt-4 border-t border-gray-100 flex flex-col items-center gap-4">
                                                    <Label htmlFor="momo-amount" className="text-sm font-bold text-gray-700">Donation Amount (UGX)</Label>
                                                    <div className="relative w-full max-w-xs">
                                                        <div className="absolute left-6 top-1/2 -translate-y-1/2 font-bold text-gray-400">UGX</div>
                                                        <Input
                                                            id="momo-amount"
                                                            type="number"
                                                            placeholder="0.00"
                                                            className="h-14 pl-16 pr-4 text-center text-xl font-bold rounded-xl border-gray-200 bg-white focus:border-blue-600 transition-all"
                                                            value={amount}
                                                            onChange={(e) => handleAmountChange(e.target.value)}
                                                            required
                                                        />
                            ...
                            <Button
                                ...
                                className="w-full h-16 rounded-full text-xl font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-200 active:scale-95 transition-all group overflow-hidden relative"
                            >
                                {isSubmitting ? (
                            ...
                            <div className="space-y-6 pt-6 border-t border-gray-100">
                                <div className="space-y-4 flex flex-col items-center">
                                    <div className="flex items-center gap-3 group cursor-pointer" onClick={() => setIsAnonymous(!isAnonymous)}>
                                        <div className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-all ${isAnonymous ? "bg-primary border-primary" : "border-gray-200 group-hover:border-primary"}`}>
                                            {isAnonymous && <CheckCircle2 className="w-4 h-4 text-white" />}
                                        </div>
                                        <span className="text-sm text-gray-600 font-medium select-none">Don't display my name publicly</span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 group">
                                        ...
                                        <span className="text-sm text-gray-600 font-medium whitespace-nowrap">Sign me up for updates from SUYEL</span>
                                    </div>
                                </div>

                                <div className="bg-blue-600 p-4 rounded-xl flex items-start gap-3 shadow-md shadow-blue-200">
                                    <ShieldCheck className="w-5 h-5 text-white shrink-0 mt-0.5" />
                                    <p className="text-xs text-white leading-relaxed">
                                        SUYEL has a <span className="font-bold underline">0% platform fee</span> for donors. Your contribution goes directly to empowering youth programs across Uganda.
                                    </p>
                                </div>
                            </div>

                            <p className="text-center text-[10px] text-gray-400 leading-relaxed max-w-sm mx-auto mt-6">
                    ...
                    <div className="lg:col-span-2">
                        <div className="sticky top-28 space-y-6">
                            <div className="bg-gray-50 rounded-3xl p-8 border border-gray-100 shadow-sm">
                                <h3 className="text-xl font-bold text-gray-900 mb-6 font-primary text-center">Your donation</h3>

                                <div className="space-y-4">
                                    <div className="flex justify-between items-center text-gray-600">
                                        <span className="font-medium">Your donation</span>
                                        <span className="font-bold">{getSymbol()} {amount ? parseFloat(amount).toLocaleString() : "0.00"}</span>
                                    </div>
                                    ...
                                    <div className="pt-4 border-t border-gray-200 flex flex-col items-center gap-2">
                                        ...
                                        <span className="text-3xl font-bold text-blue-600">{getSymbol()} {totalDue.toLocaleString()}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="px-4 text-center">
                                ...
                                <p className="text-xs text-gray-400 px-4">
                                    We use bank-level 256-bit encryption to protect your data. Payment processed via Mobile Money gateways.
                                </p>
                            </div>
                        </div>
                    </div>
